...
app = Flask(__name__)
CORS(app)
pipeline = joblib.load('modelo_simulador_web.joblib')
...
def predict():
    try:
        ...
        # --- BLOCO DE VALIDAÇÃO COMPLETO E FINAL NO BACKEND ---
        try:
            ...
            if not (0 <= filhos <= 30):
                return jsonify({'error': 'Número de filhos fora do intervalo permitido (0-30).'}), 400
            if not (0 <= tempo_trabalho_anos <= 60):
                return jsonify({'error': 'Tempo de trabalho fora do intervalo permitido (0-60 anos).'}), 400
            ...
            if not (0 < renda <= 2000000):
                return jsonify({'error': 'Renda mensal fora do intervalo permitido (R$ 1 a R$ 2.000.000).'}), 400
            if not (0 < credito <= 10000000):
                 return jsonify({'error': 'Valor de crédito solicitado fora do intervalo permitido (até R$ 10.000.000).'}), 400
            if not (0 < parcela <= 500000):
                 return jsonify({'error': 'Valor da parcela fora do intervalo permitido (até R$ 500.000).'}), 400
            if not (0 < valor_bem <= 10000000):
                 return jsonify({'error': 'Valor do bem adquirido fora do intervalo permitido (até R$ 10.000.000).'}), 400
        ...
        if scorecard_base < 400:
            ...
            score_final_prob = pipeline.predict_proba(input_df)[:, 1][0]
